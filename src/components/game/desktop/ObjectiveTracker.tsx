  ...
  easy: [
    {
      ...
      steps: [
        ...
        { key: "firewallFixed", label: "Firewall saneado", dependsOn: ["wifiFixed"] },
        { key: "malwareKilled", label: "Proceso malicioso detenido", dependsOn: ["firewallFixed"] },
      ],
    },
  ],
  medium: [
    ...
    },
    {
      title: "Cadena DNS",
      steps: [
        { key: "dnsDiagnosticsComplete", label: "Diag DNS", dependsOn: ["malwareKilled"] },
        { key: "overrideValidated", label: "Override 884", dependsOn: ["dnsDiagnosticsComplete"] },
        { key: "dnsFixed", label: "Fix DNS", dependsOn: ["overrideValidated"] },
      ],
    },
    {
      title: "Servicios & reporte",
      steps: [
        { key: "servicesVerified", label: "Verify services", dependsOn: ["dnsFixed"] },
        { key: "servicesRestarted", label: "Restart services", dependsOn: ["servicesVerified"] },
        { key: "incidentReportFiled", label: "Informe final", dependsOn: ["servicesRestarted"] },
      ],
    },
  ],
  hard: [
    ...
    },
    {
      title: "Perímetro",
      steps: [
        { key: "perimeterScanComplete", label: "Scan perimeter", dependsOn: ["malwareKilled"] },
        { key: "watchdogDeployed", label: "Deploy watchdog", dependsOn: ["perimeterScanComplete"] },
      ],
    },
    {
      title: "Switch & entrega",
      steps: [
        { key: "switchAuditComplete", label: "Audit switch", dependsOn: ["servicesRestarted"] },
        { key: "switchPortEnabled", label: "Enable port", dependsOn: ["switchAuditComplete"] },
        { key: "incidentReportFiled", label: "Informe final", dependsOn: ["watchdogDeployed", "switchPortEnabled"] },
      ],
    },
  ],
};

function resolveStatus(step: StepDefinition, progress: CaseProgress): StepStatus {
...
export default function ObjectiveTracker({
  ...
  const { sections, completion } = useMemo(() => {
    ...
    return { sections, completion: { total: steps.length, done: completed, percent } };
  }, [difficulty, progress]);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        {step.hint ? (
                          <p className="text-[0.65rem] text-white/50">{step.hint}</p>
                        ) : null}
                      </div>
                    </li>
                  );
