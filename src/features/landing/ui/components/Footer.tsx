...
import { AnimatePresence, domAnimation, LazyMotion, m } from "framer-motion"; // Importamos para el tooltip
import Image from "next/image";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { useState } from "react";
import { GitHub } from "@/features/game/ui/ui/github";
import { TextHoverEffect } from "@/features/game/ui/ui/text-hover-effect";
    ...
    >
      <AnimatePresence>
        {isHovered && (
          ...
          >
            {name}
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 size-2 bg-white rotate-45" />
          </m.div>
        )}
      </AnimatePresence>

      <Image
        className="size-10 border-2 border-zinc-900 rounded-full cursor-pointer hover:border-zinc-500 transition-colors"
        src={src}
        width={40}
        height={40}
        alt={name}
      ...
      <footer className="bg-zinc-950 border-t border-white/10 px-6 md:px-10 lg:px-20 overflow-hidden">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between py-10 text-sm text-zinc-400 font-sans gap-8">
          ...
          <div className="flex-1 flex justify-center lg:justify-end order-2 lg:order-3">
            ...
            >
              <GitHub className="size-8 lg:size-6" />
              <span className="sr-only">
                Repositorio de CaseShell en GitHub
              </span>
            </a>
          </div>
        </div>
        ...
        <div className="py-10 flex flex-col items-center justify-center text-white font-sans gap-4">
          ...
          <section className="flex -gap-x-3 rtl:space-x-reverse">
            <ContributorAvatar
              name="Alphabeath"
            ...
            <ContributorAvatar
              name="Nuitari"
              src="https://avatars.githubusercontent.com/u/11295113?v=4"
            />
          </section>
        </div>
